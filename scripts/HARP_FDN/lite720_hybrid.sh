...
run_one() {
  local tag="$1" lr="$2" obj="$3" vali="$4" msew="$5" gate="$6" slope="$7" nlin="$8"
  ...
  python -u run.py \
    ...
    --lite_linear_gate_init ${gate} --lite_linear_gate_slope_init ${slope} --lite_linear_max_gate 0.9 \
    --lite_nlinear_strength_init ${nlin} --lite_linear_dropout 0.0 \
    2>&1 | tee -a "$log"
  echo "===== $(date '+%F %T') END ${tag} =====" | tee -a "$log"
}

run_one LITE720_HYB_A_mae_lr1e4_gate0 0.00010 mae mse 0.5 0.0 0.5 -1.0
run_one LITE720_HYB_B_mix05_lr1e4_gate0 0.00010 mix mse 0.5 0.0 0.5 -1.0
run_one LITE720_HYB_C_mae_lr2e4_gate0 0.00020 mae mse 0.5 0.0 0.5 -1.0
