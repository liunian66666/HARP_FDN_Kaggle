...
class HorizonAffineCalibrator(nn.Module):
    def __init__(self, pred_len, channels, strength_init=0.0):
        super().__init__()
        self.strength = nn.Parameter(torch.tensor(float(strength_init)))
        self.scale = nn.Parameter(torch.zeros(1, pred_len, channels))
        self.bias = nn.Parameter(torch.zeros(1, pred_len, channels))

    def forward(self, y):
        gate = torch.sigmoid(self.strength)
        return y * (1.0 + gate * torch.tanh(self.scale)) + gate * self.bias


class HorizonCandidateMixer(nn.Module):
    def __init__(
        self,
        pred_len,
        channels,
        modes='last,ma4,drift4,linear4',
        strength_init=-3.1,
        max_gate=0.12,
        use_uncertainty=0,
    ):
        ...
        self.modes = [m.strip() for m in str(modes).split(',') if m.strip()]
        if not self.modes:
            self.modes = ['last']
        self.max_gate = float(max_gate)
        self.use_uncertainty = int(use_uncertainty)
        self.strength = nn.Parameter(torch.tensor(float(strength_init)))
        ...
        init_logits = torch.full((len(self.modes),), -2.0)
        init_logits[0] = 2.0
        self.candidate_logits = nn.Parameter(init_logits)

    def _candidate(self, history, mode, pred_len):
        if mode == 'last':
            return history[:, -1:, :].repeat(1, pred_len, 1)
        if mode.startswith('ma'):
            window = max(1, min(int(mode[2:]), history.size(1)))
            return history[:, -window:, :].mean(dim=1, keepdim=True).repeat(1, pred_len, 1)
        if mode.startswith('linear'):
            window = max(2, min(int(mode[6:]), history.size(1)))
        ...
        if mode.startswith('drift'):
            window = max(2, min(int(mode[5:]), history.size(1)))
            diffs = history[:, -window + 1:, :] - history[:, -window:-1, :]
            slope = diffs.mean(dim=1, keepdim=True)
            steps = torch.arange(1, pred_len + 1, device=history.device, dtype=history.dtype).view(1, -1, 1)
            return history[:, -1:, :] + steps * slope
        raise ValueError('Unsupported candidate mixer mode: {}'.format(mode))

    def forward(self, history, output, uncertainty=None):
        pred_len = output.size(1)
        candidates = [self._candidate(history, mode, pred_len) for mode in self.modes]
        stacked = torch.stack(candidates, dim=-1)
        weights = torch.softmax(self.candidate_logits, dim=0).view(1, 1, 1, -1)
        anchor = (stacked * weights).sum(dim=-1)
        gate = torch.sigmoid(self.strength)
        gate = gate * (1.0 + 0.5 * torch.tanh(self.horizon_bias[:, :pred_len, :]))
        gate = gate * (1.0 + 0.5 * torch.tanh(self.channel_bias))
        if self.use_uncertainty and uncertainty is not None:
            unc = uncertainty.detach()
            centered = unc - unc.mean(dim=1, keepdim=True)
            gate = gate * (1.0 + 0.5 * torch.tanh(self.uncertainty_scale) * centered)
        gate = gate.clamp(0.0, self.max_gate)
        return output + gate * (anchor - output)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.use_horizon_affine = int(getattr(configs, 'use_horizon_affine', 0))
        self.use_horizon_candidate_mixer = int(getattr(configs, 'use_horizon_candidate_mixer', 0))
        self.candidate_mixer_apply_in_train = int(getattr(configs, 'candidate_mixer_apply_in_train', 1))
        ...
        self.horizon_affine = None
        if self.use_horizon_affine:
            self.horizon_affine = HorizonAffineCalibrator(
                pred_len,
                c_in,
                strength_init=float(getattr(configs, 'horizon_affine_strength_init', 0.0)),
            )
        self.horizon_candidate_mixer = None
        if self.use_horizon_candidate_mixer:
            self.horizon_candidate_mixer = HorizonCandidateMixer(
                pred_len,
                c_in,
                modes=getattr(configs, 'candidate_mixer_modes', 'last,ma4,drift4,linear4'),
                strength_init=float(getattr(configs, 'candidate_mixer_strength_init', -3.1)),
                max_gate=float(getattr(configs, 'candidate_mixer_max_gate', 0.12)),
                use_uncertainty=int(getattr(configs, 'candidate_mixer_use_uncertainty', 0)),
            )
    ...
    def forward(self, x):
        ...
        if self.use_horizon_affine:
            output = self.horizon_affine(output)
        if self.use_horizon_candidate_mixer and (self.candidate_mixer_apply_in_train or (not self.training)):
            output = self.horizon_candidate_mixer(x_norm, output, uncertainty)
